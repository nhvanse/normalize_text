...
from config import VI_WORDS_PATH, UNIT_DICT, list_vietnamese_words, \
    UNIT_DICT, LSEQ_DICT,charset, url_regex, email_regex, punc
...
def replace(text):
    ...
    text, count = re.subn(
        r'(?P<id>(^| )((0)?[1-9]|[1-2][0-9]|(3)[0-1])( |)(\/)( |)(((0)?[0-9])|((1)[0-2]))( |)(\/)( |)(\d{4}|\d{2})( |$))',
    ...
    text, count = re.subn(r'(?P<id>(^| )(((0)?[0-9])|((1)[0-2]))( |)(\/)( |)(\d{4}|\d{2})( |$))',
                          lambda x: ' ' + NMON2words(''.join(x.group('id').split())) + ' ', text)
    ...
    # chuyển tiền tệ
    text, count = re.subn(r'(?P<id>(\d+ \. )*\d+( \, \d+)? ({}))( |$)'.format(currency_list_regex),
    ...
    # chuyển giờ
    text, count = re.subn(
        r'(?P<id>(((0|1)?[0-9]|(2)[0-3]):([0-5]?[0-9]):([0-5]?[0-9]))|(((0|1)?[0-9]|(2)[0-3]):([0-5]?[0-9])))',
        lambda x: NTIM2words(x.group('id')),
        text)
    ...
    for i, sub_token in enumerate(sub_tokens):
        if sub_token.lower() not in vn_words_dict:
            ...
            sub_token, count = re.subn(
                r'(?P<id>(\d+\.)*\d+(\,\d+)?)', lambda x: NNUM2words(''.join(x.group('id').split('.'))) + ' ',
                sub_token)
            nswcounter['NNUM'] += count
            # chuyển punctuation
...
t0 = time()
print(normalize(open('./vidu.txt').read()))
print(time()-t0)